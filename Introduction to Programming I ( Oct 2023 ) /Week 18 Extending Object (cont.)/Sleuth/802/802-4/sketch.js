...
Officer: 6174876
CaseNum: 802-3-89394806-6174876
...
- First write a shuffleSeed() function. Same as before, it needs to return an array of 52 random integers between 4 and 76.
...
- Once you've found a match use the JavaScript splice() function to store the cut card and all the elements after from the cards array in the deckUptoCut array. 
...
function deck_cut() {
  ...
  for (var i = 0; i < cards.length; i++) {
    if (cards[i].suit == cutPoint.type && cards[i].number == cutPoint.no) {
      deckUptoCut = cards.splice(i);
      break;
    }
  }
  //reverse the elements in deckUptoCut
  var tempDeck = [];
...
function shuffleSeed() {
  var shuffleSeed = [];
  for (var i = 0; i < 52; i++) {
	shuffleSeed.push(round(random(4, 76)));
  }
  return shuffleSeed;
}
