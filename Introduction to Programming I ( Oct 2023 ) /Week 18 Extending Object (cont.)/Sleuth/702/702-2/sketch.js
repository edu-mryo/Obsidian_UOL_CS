...
function MoveVehicle() {
  /*
		...
		*/
  investigatorVehicleObject.milesTravelled +=
    investigatorVehicleObject.speedAmt;
  investigatorVehicleObject.engineShudderVal += random(-0.05, 0.05);
  investigatorVehicleObject.engineShudderVal = constrain(
    investigatorVehicleObject.engineShudderVal,
    0.05,
    1.05
  );
  TurnMotor(investigatorVehicleObject);
}
...
function MoveLanes(car) {
  /*
		...
		*/
  if (car.coordinateX == Lane_CoordA) {
    car.coordinateX = Lane_CoordB;
  } else {
    car.coordinateX = Lane_CoordA;
  }
}
function SearchCarIsAhead(TargetVehicleA, TargetVehicleB) {
  /*
		...
		*/
  if (
    TargetVehicleA.coordinateX == TargetVehicleB.coordinateX &&
    TargetVehicleB.milesTravelled - TargetVehicleA.milesTravelled < 200 &&
    TargetVehicleB.milesTravelled - TargetVehicleA.milesTravelled >= 0
  ) {
    return true;
  } else {
    return false;
...
function draw() {
  ...
  for (var i = 0; i < CarsList.length; i++) {
    ...
    CarsList[i].coordinateY =
      investigatorVehicleObject.coordinateY -
      CarsList[i].milesTravelled +
      investigatorVehicleObject.milesTravelled;
  }
}
...
function drawRoad() {
  ...
  for (var i = -1; i < 20; i++) {
    line(
      ...
      i * 100 + (investigatorVehicleObject.milesTravelled % 100),
      roadLeftEdge + roadWidth / 2,
      i * 100 + 70 + (investigatorVehicleObject.milesTravelled % 100)
    );
  }
}
...
function drawCars() {
  ...
  image(
    carImages["detective"],
    investigatorVehicleObject.coordinateX -
  ...
  for (var i = 0; i < CarsList.length; i++) {
    if (
      CarsList[i].coordinateY < height &&
      CarsList[i].coordinateY > -height / 2
    ) {
      image(
...
function TurnMotor(car) {
  car.exhaust.push({
    size: 2,
    x: car.coordinateX,
    y: car.coordinateY + carImages[car.vehicleType].height,
  });

  for (var i = car.exhaust.length - 1; i >= 0; i--) {
    car.exhaust[i].y += max(0.75, car.speedAmt / 3);
    if (car.vehicleType != "detective")
      car.exhaust[i].y += investigatorVehicleObject.speedAmt - car.speedAmt;
