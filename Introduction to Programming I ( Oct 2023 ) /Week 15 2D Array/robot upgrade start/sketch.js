...
var highestIq = 0;
var highestIqIndex;
var katanFound = false;
var lowestSerialNumber;
var lowestSerialNumberIndex;
var humanDestroyer;
var humanDestroyerIndex;
var narrowFace =[];
...
function setup() {
	...
	fillRobots();
	lowestSerialNumber = robots[0].serialNumber
	background(20);
	...
	for (var i = 0; i < robots.length; i++) {
		...
		if (robots[i].serialNumber < lowestSerialNumber) {
			lowestSerialNumber = robots[i].serialNumber;
			lowestSerialNumberIndex = i;
		}

		for(var j=0;j<robots[i].abilities.length;j++){
			if(robots[i].abilities[j] =="destroy humanity"){
				humanDestroyerIndex = i
			}
		}
	

		// Check if last name is Katan
		if (!katanFound && robots[i].secondName == 'Katan') {
			highlight(robots[i], 0, 255, 0);
			katanFound = true;
		}
		

		// Print names unless a Katan is found
		if (!katanFound) {
		...
		if(robots[i].physical.headWidth<70){
			// console.log(i)
			console.log(robots[i].physical.headWidth)
			narrowFace.push(i);
		}
	...
	highlight(robots[highestIqIndex], 0, 0, 255);
	console.log(`The robot with the highest IQ is called ${robots[highestIqIndex].firstName} ${robots[highestIqIndex].secondName} and its IQ is ${highestIq}`)

	//4. Select the robot with the smallest serial number and highlight it in purple, Also, print it's name to the console
	highlight(robots[lowestSerialNumberIndex], 128, 0, 128);
	console.log(`The robot with the lowest Serial Number is called ${robots[lowestSerialNumberIndex].firstName} ${robots[lowestSerialNumberIndex].secondName} and its serial number is ${lowestSerialNumber}`)

	//5. Find the robot who want to "destroy humanity" highlight it in red. Also, print it's name to the console
	highlight(robots[humanDestroyerIndex],255,0,0);
	console.log(`The robot who wants  to destroy humanity is ${robots[humanDestroyerIndex].firstName} ${robots[humanDestroyerIndex].secondName}`)
	//6. Find any robot whose face is narrower than 70 and highlight it in yellow. Also, print it's name to the console
	for( var i = 0; i<narrowFace.length;i++){
		highlight(robots[narrowFace[i]],255,255,0)
	}
